<main>
    <div class="container">
        ...
        <?php if ((isset($_GET['students']) && $_GET['students'] != "") && (isset($_GET['courses']) && $_GET['courses'] != "")) { ?>
            ...
            $query = "SELECT * FROM marks m INNER JOIN modules md ON md.id=m.module_id WHERE m.student_id=$student AND md.course_id=$courses $semister";
            $studentrow = mysqli_query($connection, "SELECT u.*,l.id as levelid,l.level FROM users u INNER JOIN levels l ON l.id=u.level_id WHERE u.id=$student LIMIT 1")->fetch_array();
                    ...
                    <table class="table table-striped table-hover table-primary">
                        ...
                        <tbody>
                            ...
                            while ($row = $exquery->fetch_array()) {
                                ...
                                <th>' . $row['semister'] . '</th>
                                <th> <a href="index.php?route=/pages/marks/releasedresult&name=Released Result&student=' . $row['id'] . '&semister=' . $row['semister'] . '&course=' . $courses . '">**...Released...**</a></th>
